...
{
    int clientfd;                    // 클라이언트 소켓을 위한 파일 디스크립터(파일처럼 읽고/쓰는 대상이 됨)
    char *host, *port, buf[MAXLINE]; // 서버 주소(IP), 포트 번호, 데이터를 담을 버퍼
     ...
     *    성공하면 서버와 연결된 소켓의 번호표, 즉 파일 디스크럽터(clientfd)를 반환
     */
    clientfd = Open_clientfd(host, port);

    /*
     * 2. RIO 읽기 버퍼를 초기화.
     *    우리가 만든 파일 디스크럽터(clientfd)와 RIO 구조체(rio)를 연결해주는 작업
     *    이제 rio라는 도구를 통해 clientfd에서 데이터를 안정적으로 읽을 수 있게 됨 
     */
    ...
    while (Fgets(buf, MAXLINE, stdin) != NULL) {
         ...
         *    Rio_writen은 데이터(buf)를 clientfd에 연결된 서버로 확실하게(n 바이트 전부) 보냄
         */
        Rio_writen(clientfd, buf, strlen(buf));

        /*
         * 5. 서버가 보낸 "에코" 데이터를 다시 읽어들임
         *    Rio_readlineb는 rio 버퍼를 통해 서버로부터 한 줄(\n을 만날 때까지)을 읽어 buf에 저장
         */
...
// 2. 연결: Open_clientfd로 서버에 TCP 연결을 요청하고, 통신용 번호표(clientfd)를 받는다.
