...
int main(int argc, char **argv)
{
    ...
    // sockaddr_storage: 어떤 형태의 주소(IPv4, IPv6 등)든 충분히 담을 수 있는 큰 구조체
    struct sockaddr_storage clientaddr;
    char client_hostname[MAXLINE], client_port[MAXLINE]; // getnameinfo 결과 문자열을 담을 버퍼
    ...
    /* 
     * 1. 듣기 소켓을 생성하고 준비시킴 
     *    Open_listenfd는 내부적으로 socket(), bind(), listen() 시스템 콜을 차례로 실행해줌
     *    argv[1] (포트 번호)에서 클라이언트의 연결 요청을 받을 준비를 마침
     ...
     */
    while (1) {
        clientlen = sizeof(struct sockaddr_storage); // accept()를 호출하기 전, 주소 구조체의 크기를 초기화

        /*
         * 3. 클라이언트의 연결 요청을 수락(Accept)함
         *    Accept 함수는 클라이언트가 연결을 요청할 때까지 여기서 '대기(block)'함 
         *    연결 요청이 오면, 클라이언트와 통신할 새로운 '연결 소켓(connfd)'을 만들어 반환하고,
         *    clientaddr 구조체에는 클라이언트의 주소 정보를 채워 넣음
...
// 전체 요약
// 1. 개업 준비: Open_listenfd로 특정 포트에 귀를 기울이는 듣기 소켓(listenfd)을 열어놓음
// 2. 영업 시작 (무한 루프)
