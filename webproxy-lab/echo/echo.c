...
{
    size_t n;          // 읽어온 데이터의 바이트 수를 저장할 변수. size_t는 'unsigned long'과 비슷한 타입으로, 크기를 나타낼 때 주로 사용됨
     ...
     *    Rio_readlineb 함수는 connfd로부터 한 줄을 읽어 buf에 저장하고, 읽은 바이트 수를 반환
     *    클라이언트가 연결을 끊으면(EOF), Rio_readlineb는 0을 반환하고 루프는 종료됨
     */
